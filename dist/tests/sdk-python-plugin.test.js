...
import { createToolRegistry, InvalidPluginError, loadPythonPlugins, makePythonPluginHandler, PythonPluginError, PythonPluginTimeoutError, } from "../src/sdk/index.js";
...
describe("python plugin loader — discovery (G3b)", () => {
    ...
    it("rejects malformed sidecar schemas with the path and reason", async () => {
        ...
        for (const [name, schema, reason] of cases) {
            ...
            await assert.rejects(loadPythonPlugins(specFor(agentDir, [name]), createToolRegistry()), (err) => {
