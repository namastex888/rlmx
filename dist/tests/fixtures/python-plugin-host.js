...
import { makePythonPluginHandler, PythonPluginError, PythonPluginTimeoutError, } from "../../src/sdk/python-plugin.js";
...
async function script(name, source) {
    const path = join(root, `${name}.py`);
    await writeFile(path, source);
    return path;
}
function noAbortListeners(signal = ctx.signal) {
...
try {
    ...
    for (let i = 0; i < 3; i++) {
        await assert.rejects(makePythonPluginHandler("early", early)(largeArgs, ctx), (error) => {
    ...
    await assert.rejects(makePythonPluginHandler("zero", zero)(largeArgs, ctx), (error) => {
        assert.ok(error instanceof PythonPluginError);
    ...
    await assert.rejects(makePythonPluginHandler("nonzero", nonzero)({}, ctx), (error) => {
        assert.ok(error instanceof PythonPluginError);
...
//# sourceMappingURL=python-plugin-host.js.map