...
function updaterEntry(file: string) {
...
interface Invalid { name: string; field: string; mutate: (root: string) => void }
const writeJson = (path: string, value: unknown) => writeFileSync(path, JSON.stringify(value));
function edit(root: string, file: string, fn: (json: Json) => void) {
  const path = join(root, file); const value = JSON.parse(readFileSync(path, "utf8")); fn(value); writeJson(path, value);
}
...
for (const file of ["package.json", "package-lock.json"]) {
  ...
  for (const value of [null, [], 7]) invalid.push({ name: `${file} is ${JSON.stringify(value)}`, field: file, mutate: root => writeJson(join(root, file), value) });
}
...
for (const value of [null, [], 7]) invalid.push({ name: `root package is ${JSON.stringify(value)}`, field: 'packages[""]', mutate: root => edit(root, "package-lock.json", l => { l.packages[""] = value; }) });
for (const field of ["name", "version"]) {
  for (const atRoot of [false, true]) invalid.push({ name: `${field} mismatch at ${atRoot ? "root package" : "lock top level"}`, field: atRoot ? `packages[""].${field}` : `package-lock.json.${field}`, mutate: root => edit(root, "package-lock.json", l => { (atRoot ? l.packages[""] : l)[field] = "different"; }) });
}
for (const field of ["dependencies", "devDependencies", "engines"]) invalid.push({ name: `${field} mismatch`, field, mutate: root => edit(root, "package-lock.json", l => { l.packages[""][field] = { changed: "1" }; }) });
for (const file of competing) invalid.push({ name: `competing ${file}`, field: file, mutate: root => writeFileSync(join(root, file), "competing lock\n") });

type Seam = "local wrapper body adapter" | "CI command body adapter" | "canonical installer" | "source updater entry adapter" | "committed updater entry adapter" | "launcher";
const seams: Seam[] = ["local wrapper body adapter", "CI command body adapter", "canonical installer", "source updater entry adapter", "committed updater entry adapter", "launcher"];
function git(cwd: string, args: string[]) {
  const r = spawnSync("git", args, { cwd, encoding: "utf8" }); assert.equal(r.status, 0, r.stderr); return r.stdout.trim();
}
function put(path: string, value = SENTINEL) { mkdirSync(dirname(path), { recursive: true }); writeFileSync(path, value); }
function complete(root: string) {
  for (const path of [".package-lock.json", "js-yaml/package.json", "@earendil-works/pi-ai/package.json"]) put(join(root, "node_modules", path), "{}");
}
function snapshot(path: string): unknown {
  if (!existsSync(path)) return null;
  const stat = lstatSync(path);
  if (stat.isSymbolicLink()) return { link: readlinkSync(path) };
  if (!stat.isDirectory()) return { bytes: readFileSync(path).toString("base64"), mode: stat.mode };
...
function fixture(mutate?: (root: string) => void) {
  ...
  mkdirSync(remote); mkdirSync(bins);
  cpSync(join(ROOT, "bin"), join(remote, "bin"), { recursive: true });
  mkdirSync(join(remote, "scripts"));
  for (const file of ["install.sh", "check-npm-authority.mjs"]) cpSync(join(ROOT, "scripts", file), join(remote, "scripts", file));
  ...
  git(remote, ["init", "-b", "main"]); git(remote, ["config", "user.name", "Fixture"]); git(remote, ["config", "user.email", "fixture@example.invalid"]);
  git(remote, ["add", "."]); git(remote, ["commit", "-m", "fixture base"]);
  const base = git(remote, ["rev-parse", "HEAD"]);
  mutate?.(remote); put(join(remote, "selected-candidate"), "candidate");
  git(remote, ["add", "-A"]); git(remote, ["commit", "-m", "fixture selected authority"]);
...
`);
  ...
  function run(seam: Seam, advance = true) {
    let cmd = process.execPath; let args: string[];
    if (seam.includes("updater") && advance) git(root, ["reset", "--hard", base]);
    if (seam === "local wrapper body adapter" || seam === "CI command body adapter") {
      assert.equal(wrapper, expectedWrapper);
      if (seam === "CI command body adapter") assert.equal(ciCommand, "npm run deps:ci");
      cmd = "bash"; args = ["-c", wrapper];
      ...
      cmd = "bash"; args = [join(ROOT, "scripts/install.sh")];
    ...
    else args = [join(root, "bin/mikro.mjs"), "--version"];
    const r = spawnSync(cmd, args, { cwd: root, env, encoding: "utf8", timeout: 20_000 });
    assert.ifError(r.error);
    if (seam.includes("updater") || seam === "canonical installer") assert.equal(git(root, ["rev-parse", "HEAD"]), target, `actual selection must reach candidate: ${r.stdout}${r.stderr}`);
    return { status: r.status, out: r.stdout + r.stderr };
  }
  function calls(): string[][] { return existsSync(events) ? readFileSync(events, "utf8").trim().split("\n").map(line => JSON.parse(line)) : []; }
...
function rejected(result: { status: number | null; out: string }, field: string) {
...
it("wrapper and actual CI installation command have the frozen identities", () => {
  assert.equal(wrapper, expectedWrapper); assert.equal(ciCommand, "npm run deps:ci");
  assert.doesNotMatch(workflow, /run: npm ci\b/);
});
for (const seam of seams) describe(seam, () => {
  for (const bad of invalid) it(bad.name, { timeout: 30_000 }, () => {
    const f = fixture(bad.mutate);
    try { const before = f.trees(); rejected(f.run(seam), bad.field); assert.deepEqual(f.trees(), before); assert.deepEqual(f.calls(), []); }
    finally { f.close(); }
  });
  it("coherent control reaches exactly one npm installation", { timeout: 30_000 }, () => {
    const f = fixture();
    try {
      const r = f.run(seam); assert.equal(r.status, 0, r.out);
      ...
      if (seam === "launcher") assert.match(r.out, /AUTHORITY_CLI_READY/);
    } finally { f.close(); }
  });
});
...
it("real npm run deps:ci dispatcher reaches exactly one fake child installation", () => {
  ...
  try {
    ...
    assert.equal(r.status, 0, r.stdout + r.stderr); assert.deepEqual(f.calls(), [["ci"]]);
  } finally { f.close(); }
});

for (const seam of seams.slice(2)) for (const parked of ["legacy", "sibling"]) for (const ready of [false, true]) for (const valid of [false, true]) {
  it(`${seam}: ${valid ? "coherent" : "invalid"} ${parked} recovery, ${ready ? "complete" : "incomplete"} current`, { timeout: 30_000 }, () => {
    ...
    try {
      ...
      const before = f.trees(); const r = f.run(seam);
      if (valid) { assert.equal(r.status, 0, r.out); assert.equal(f.calls().filter(a => a[0] === "ci").length, 1); }
      else { rejected(r, "dependencies"); assert.deepEqual(f.trees(), before); assert.deepEqual(f.calls(), []); }
    } finally { f.close(); }
  });
}

for (const valid of [false, true]) it(`custom-prefix legacy migration: ${valid ? "both renames remain reachable" : "invalid selected root preserves both paths and trees"}`, () => {
  ...
  try {
    ...
    if (!valid) {
      rejected(r, "engines"); assert.deepEqual([f.trees(), snapshot(join(f.area, ".rlmx")), snapshot(join(f.area, ".mikro"))], before); assert.deepEqual(f.calls(), []);
    } else {
      assert.equal(r.status, 0, r.out); assert.equal(existsSync(join(f.area, ".rlmx")), false);
...
for (const control of ["absent versus empty", "nested key order", "excluded fields and nested locks"]) it(`exact authority boundary: ${control}`, () => {
  const f = fixture(root => {
    if (control === "absent versus empty") {
      ...
      for (const name of competing) put(join(root, "nested", name));
      put(join(root, "nested/package.json"), "invalid consumer metadata");
    }
  });
  try { const r = f.run("local wrapper body adapter"); assert.equal(r.status, 0, r.out); assert.deepEqual(f.calls(), [["ci"]]); }
  finally { f.close(); }
});
