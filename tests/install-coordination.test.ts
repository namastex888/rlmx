...
interface ProcessRun { child: ChildProcess; done: Promise<{ code: number | null; out: string }>; output: () => string }
function start(cwd: string, cmd: string, args: string[], env: NodeJS.ProcessEnv): ProcessRun {
  const child = spawn(cmd, args, { cwd, env, stdio: ["ignore", "pipe", "pipe"] });
  let out = "";
  child.stdout!.on("data", c => { out += c; }); child.stderr!.on("data", c => { out += c; });
  const done = new Promise<{ code: number | null; out: string }>((resolve, reject) => {
...
async function fixture(actualCli = false) {
  ...
  const remote = join(area, "remote"); const checkout = join(area, "checkout"); const bins = join(area, "bin");
  ...
  if (actualCli) await cp(join(ROOT, "dist/src"), join(remote, "dist/src"), { recursive: true });
  command(remote, "git", ["init", "-b", "main"]); command(remote, "git", ["config", "user.name", "Fixture"]); command(remote, "git", ["config", "user.email", "fixture@example.invalid"]);
  command(remote, "git", ["add", "."]); command(remote, "git", ["commit", "-m", "fixture"]);
  command(area, "git", ["clone", remote, checkout]);
  await writeFile(join(bins, "npm"), NPM); await chmod(join(bins, "npm"), 0o755);
  ...
  function launch(args: string[] = ["--version"], extra: NodeJS.ProcessEnv = {}) {
    const p = start(checkout, process.execPath, [join(checkout, "bin/mikro.mjs"), ...args], { ...env, ...extra }); processes.push(p); return p;
  }
  async function complete() {
    for (const name of [".package-lock.json", "js-yaml/package.json", "@earendil-works/pi-ai/package.json", "sentinel"]) {
      const path = join(checkout, "node_modules", name); await mkdir(dirname(path), { recursive: true }); await writeFile(path, name === "sentinel" ? "old" : "{}");
    }
  }
  ...
  async function close() {
    ...
    await rm(join(area, "ci-gate"), { force: true }); await rm(join(area, "build-gate"), { force: true });
    for (const p of processes) if (p.child.exitCode === null && p.child.signalCode === null) p.child.kill("SIGTERM");
...
it("updater and launcher serialize npm/build and recheck completion after waiting", { timeout: 30_000 }, async () => {
  ...
  try {
    ...
    await f.advance(); await writeFile(join(f.area, "ci-gate"), "");
...
it("live-owner contention is bounded and canonical installer cannot reset its checkout", { timeout: 30_000 }, async () => {
  ...
  try {
    await writeFile(join(f.area, "ci-gate"), ""); const owner = f.launch();
    ...
    assert.deepEqual(await f.events(), ["ci:start"]);
    await rm(join(f.area, "ci-gate")); assert.equal((await owner.done).code, 0, owner.output());
  } finally { await f.close(); }
});
...
it("failed npm restores the previous tree and retry repairs the unchanged SHA", { timeout: 30_000 }, async () => {
  const f = await fixture();
  try {
    await f.complete(); await f.advance(); await writeFile(join(f.area, "ci-fail"), "");
    const first = f.launch(["update"]); assert.equal((await first.done).code, 23, first.output());
    assert.equal(await readFile(join(f.checkout, "node_modules/sentinel"), "utf8"), "old");
    await rm(join(f.area, "ci-fail")); const retry = f.launch(["update"]);
...
it("failed build retries at unchanged SHA without another npm writer or partial CLI import", { timeout: 30_000 }, async () => {
  const f = await fixture();
  try {
    await f.complete(); await f.advance(); await writeFile(join(f.area, "build-fail"), "");
    const first = f.launch(["update"]); assert.equal((await first.done).code, 42, first.output());
    const sha = command(f.checkout, "git", ["rev-parse", "HEAD"]);
    await rm(join(f.area, "build-fail")); const retry = f.launch(["update"]);
    assert.equal((await retry.done).code, 0, retry.output());
    assert.equal(command(f.checkout, "git", ["rev-parse", "HEAD"]), sha);
    const version = f.launch(); assert.equal((await version.done).code, 0, version.output());
...
it("dead owner recovery waits for its surviving npm child before acquiring mutation ownership", { timeout: 30_000, skip: process.platform === "win32" }, async () => {
  const f = await fixture();
  try {
    await f.complete(); await f.advance(); await writeFile(join(f.area, "ci-gate"), "");
    ...
    await first.done;
    const recovered = f.launch(); assert.equal((await recovered.done).code, 0, recovered.output());
...
it("interrupted build is recovered by launch even when dist and dependencies exist", { timeout: 30_000, skip: process.platform === "win32" }, async () => {
  const f = await fixture();
  try {
    await f.complete(); await f.advance(); await writeFile(join(f.area, "build-gate"), "");
    const first = f.launch(["update"]);
    await until(() => existsSync(join(f.area, "build-ready")), "no build owner");
    const owner = JSON.parse(await readFile(f.checkout + ".mikro-install/owner.json", "utf8"));
    process.kill(owner.pid, "SIGKILL"); await rm(join(f.area, "build-gate"));
    await first.done;
    const recovered = f.launch(); assert.equal((await recovered.done).code, 0, recovered.output());
  ...
  try {
    for (const shutdown of ["eof", "signal"]) {
      ...
      try {
        ...
        if (shutdown === "eof") assert.equal(result.code, 0, err); else assert.equal(result.signal, "SIGTERM");
...
it("launcher protects asynchronous CLI startup from an updater parking dependencies", { timeout: 30_000 }, async () => {
  ...
  try {
    await f.complete();
    const gate = join(f.area, "startup-gate"); await writeFile(gate, "");
...
it("legacy-only default-prefix install refuses before creating state or mutating data and symlinks", { timeout: 30_000 }, async () => {
  const f = await fixture();
  try {
    const legacy = join(f.area, ".rlmx"); const oldBin = join(f.area, ".local/bin");
    const oldLauncher = join(legacy, "rlmx/bin/mikro.mjs");
    await mkdir(dirname(oldLauncher), { recursive: true }); await mkdir(oldBin, { recursive: true });
...
it("real ACP lifetime does not block version or concurrent MCP startup", { timeout: 30_000 }, async () => {
  ...
  try {
    await symlink(join(ROOT, "node_modules"), join(f.checkout, "node_modules"), "dir");
    function protocol(command: "acp" | "mcp") {
      ...
      child.stdout.on("data", c => { out += c; }); child.stderr.on("data", c => { err += c; });
      const done = new Promise<{ code: number | null; out: string }>((resolve, reject) => {
        child.on("error", reject); child.on("close", code => resolve({ code, out: out + err }));
      });
    ...
    mcp.child.stdin.end(); assert.equal((await mcp.done).code, 0);
    acp.child.stdin.end(); assert.equal((await acp.done).code, 0);
  } finally { await f.close(); }
});
...
for (const mode of ["cache", "batch", "cost", "oolong"]) it(`${mode} model work after preparation does not own the installation mutex`, { timeout: 30_000 }, async () => {
  const f = await fixture(true);
  try {
    await symlink(join(ROOT, "node_modules"), join(f.checkout, "node_modules"), "dir");
    ...
    const input = join(f.area, "input.md"); await writeFile(input, "A fixture question and context.\n");
